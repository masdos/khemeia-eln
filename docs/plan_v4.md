...
* **Capa de Interfaz (UI):** NiceGUI en modo escritorio (`native=True`). Gestiona formularios complejos, estado mutable y widgets condicionales.
...
`ai_provider` se inicializa como `null` y se configura desde la página AI Assistant cuando el usuario activa esa funcionalidad por primera vez. No es un campo obligatorio del formulario de bienvenida.

`user_name` queda disponible para los prompts de AIService sin necesidad de solicitarlo en cada sesión. El usuario puede modificar su perfil desde la pantalla de ajustes en cualquier momento.
